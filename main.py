...
def main():
    ...
    if player_char not in ('B', 'W'):
        print("Error: Player must be either 'B' or 'W'")
        return
    
    with open(board_file, 'r') as f:
        board_state = [list(line.strip().split()) for line in f.readlines()]
   
    if len(board_state[-1]) < len(board_state[0]):
    ...
    best_score, best_move_node = game.minimax(game.root, depth=2, maximizing_player=True)
    
    if best_move_node:
        move_representation = game.format_move(game.root.state, best_move_node.state)
        print(f"Best move: {move_representation} (score: {best_score})")
    else:
